...
describe('SaltyForwarder', function () {
  ...
  it('should forward with salt', async () => {
    ...
    const forwardRequest: SaltyForwarder.ForwardRequestStruct =
      await buildForwardRequest(oracleId, callback);
    const signedRequest = await signForwardRequest(forwardRequest);

    expect(await forwarderContract.verify(forwardRequest, signedRequest)).is
      .true;
    await expect(
      forwarderContract.execute(forwardRequest, signedRequest)
    ...
    expect(
      await forwarderContract.isConsumedSalt(
        reporterAccount.address,
        forwardRequest.salt
      )
    ).is.true;
  ...
  it('should sign metatxs for different oracles and forward them later', async () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const tx = forwarderContract.execute(
        requestAndSignature[0],
        requestAndSignature[1]
      );
      await expect(tx).to.emit(singleReporterOracle, 'ValueUpdated');
      const { date: foundDate, stringValue: foundValue } =
        await singleReporterOracle.getString(oracleId);
      expect(date).equal(foundDate);
      expect(value).equal(foundValue);
  ...
  async function buildForwardRequest(
    oracleId: string,
    callback: string
  ): Promise<SaltyForwarder.ForwardRequestStruct> {
    const gas = await singleReporterOracle
    ...
    return {
      ...
      data: singleReporterOracle.interface.encodeFunctionData('receiveResult', [
        oracleId,
        callback,
      ]),
    };
  ...
  async function signForwardRequest(
    forwardRequest: SaltyForwarder.ForwardRequestStruct
  ) {
