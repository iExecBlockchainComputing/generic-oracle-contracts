...
describe("SingleReporterOracle", function () {
    ...
    beforeEach("Fresh contract & accounts", async () => {
        ...
        singleReporterOracle = await SingleReporterOracleFactory.deploy(reporterAccount.address, noForwarder)
    });

    it('should not construct when authorized reporter is 0x0', async () => {
        const SingleReporterOracleFactory = await ethers.getContractFactory("SingleReporterOracle")
        await expect(SingleReporterOracleFactory.deploy(ethers.constants.AddressZero, noForwarder))
            .to.be.reverted;
    });

    it('should construct with custom address as authorized reporter', async () => {
        expect(await singleReporterOracle.authorizedReporter()).equal(reporterAccount.address)
    });
    ...
    it('should not receive since reporter not authorized', async () => {
        ...
        await expect(singleReporterOracle.connect(defaultAccount).receiveResult(oracleId, callback))
    ...
    it('should receive and get timed string value', async () => {
        ...
        await expect(singleReporterOracle.connect(reporterAccount).receiveResult(oracleId, callback))
    ...
    it('should get timed int value', async () => {
        ...
        await singleReporterOracle.connect(reporterAccount).receiveResult(oracleId, callback)

        const { date: foundDate, intValue: foundValue } = await singleReporterOracle.getInt(oracleId);
        expect(date).equal(foundDate);
        expect(value).equal(foundValue);
    });
    ...
    it('should get timed bool value', async () => {
        ...
        const { date: foundDate, boolValue: foundValue } = await singleReporterOracle.getBool(oracleId);
        expect(date).equal(foundDate);
    ...
    it('should get timed bytes value', async () => {
        ...
        const { date: foundDate, bytesValue: foundValue } = await singleReporterOracle.getRaw(oracleId);
        expect(date).equal(foundDate);
        expect(value).equal(foundValue);
    });
