...
async function main() {
    ...
    if ((chainId == 1 || chainId == 137)
        && authorizedReporter.toLowerCase() != '0xa1135C5f1309eF9836679d31d7bea9846827f699'.toLowerCase()) {
        console.log("Do you really want to deploy SingleReporterOracle on network:%s with reporter:%s?", 
            chainId, authorizedReporter)
        exit(1);
    }
    ...
    console.log("SaltyForwarder: %s [tx:%s]", 
        forwarder.address, forwarder.deployTransaction.hash);
    ...
    console.log("SingleReporterOracle: %s [tx:%s]", 
        singleReporterOracle.address, singleReporterOracle.deployTransaction.hash);
    ...
    console.log("npx hardhat verify --network %s %s", 
        chainId, forwarder.address)
    ...
    console.log("npx hardhat verify --network %s %s %s %s", 
        chainId, singleReporterOracle.address, authorizedReporter, forwarder.address)
}
